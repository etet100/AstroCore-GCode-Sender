...
void StateBehaviorGarbageCollector::track(StateBehavior *sb)
{
    ...
    for (int i = 0; i < m_trackedBehaviors.size(); ++i) {
        if (m_trackedBehaviors[i] == sb) {
            ...
            qDebug() << "[Behavior][GC] Moved" << sb->description() << "to front, total tracked:" << m_trackedBehaviors.count();
            return;
        }
    }
...
void StateBehaviorGarbageCollector::cleanup()
{
    ...
    if (deleted > 0) {
        qDebug() << "[Behavior][GC] Cleanup complete: deleted" << deleted << "behaviors, retained" << m_trackedBehaviors.count();
    }
}
