...
void StateBehaviorGarbageCollector::track(StateBehavior *sb)
{
    ...
    for (int i = 0; i < m_trackedBehaviors.size(); ++i) {
        if (m_trackedBehaviors[i] == sb) {
            ...
            qDebug() << "[GC] Moved" << sb->description() << "to front, total tracked:" << m_trackedBehaviors.count();
            return;
        }
    }

    m_trackedBehaviors.prepend(QPointer<StateBehavior>(sb));
    qDebug() << "[GC] Tracking new behavior" << sb->description() << ", total tracked:" << m_trackedBehaviors.count();
}
...
void StateBehaviorGarbageCollector::cleanup()
{
    ...
    while (m_trackedBehaviors.size() > m_maxRetainedObjects) {
        ...
        if (!sb.isNull()) {
            qDebug() << "[GC] Deleting" << sb->description();
            delete sb.data();
        }
    }
    ...
    if (deleted > 0) {
        qDebug() << "[GC] Cleanup complete: deleted" << deleted << "behaviors, retained" << m_trackedBehaviors.count();
    }
}
