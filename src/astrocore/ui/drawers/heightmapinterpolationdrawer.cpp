...
bool HeightMapInterpolationDrawer::updateData(GLPalette &palette)
{
    ...
    if (!m_data || m_data->count() == 0 || m_data->at(0).count() == 0) {
        m_lines.clear();
        return true;
    }
    ...
    // Flat surface (or no data at all) would divide by zero in the color mapping.
    double range = max - min;
    if (qIsNaN(range) || qFuzzyIsNull(range)) {
        range = 1.0;
    }
    ...
    for (int i = 0; i < interpolationPointsY; i++) {
        for (int j = 1; j < interpolationPointsX; j++) {
            // Both ends must be known — a NaN would end up in the vertex position.
            if (qIsNaN(m_data->at(i).at(j)) || qIsNaN(m_data->at(i).at(j - 1))) continue;
            color.setHsvF(0.67 * (max - m_data->at(i).at(j - 1)) / range, 1.0, 1.0);
            ...
            color.setHsvF(0.67 * (max - m_data->at(i).at(j)) / range, 1.0, 1.0);
            vertex.color = palette.color(color);
    ...
    for (int j = 0; j < interpolationPointsX; j++) {
        for (int i = 1; i < interpolationPointsY; i++) {
            if (qIsNaN(m_data->at(i).at(j)) || qIsNaN(m_data->at(i - 1).at(j))) continue;
            color.setHsvF(0.67 * (max - m_data->at(i - 1).at(j)) / range, 1.0, 1.0);
            vertex.color = palette.color(color);
...
void HeightMapInterpolationDrawer::setInterpolationMode(Heightmap::InterpolationMode mode)
{
