...
class GcodePreprocessorUtils : public QObject
{
    Q_OBJECT
public:
    static GCodeItem parseLine(const QString &line);
    static GCodeItem parseLine(std::string line); // legacy, prefer QString overload
    static void parseLines(QStringList& lines, GCode& gcode);
    ...
    static QList<float> parseCodes(const std::vector<std::string> &args, char code);
    static QList<int> parseGCodes(QString command);
    static QList<int> parseMCodes(QString command);
    static std::vector<std::string> splitCommand(const QString &command);
    static std::vector<std::string> splitCommand(const std::string &command);
    static double parseCoord(const std::vector<std::string> &argList, char c);
    static QVector3D updatePointWithCommand(const QVector3D &initial, double x, double y, double z, bool absoluteMode);
    static QVector3D updatePointWithCommand(const std::vector<std::string> &commandArgs, const QVector3D &initial, bool absoluteMode);
    ...
    static QVector3D updateCenterWithCommand(const std::vector<std::string> &commandArgs, QVector3D initial, QVector3D nextPoint, bool absoluteIJKMode, bool clockwise);
