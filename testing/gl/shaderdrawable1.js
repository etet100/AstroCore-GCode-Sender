    ...
    draw(eye) {
        ...
        if (this.once) {
            for (let i = 0; i < vertexData__.length; i += 9) {
                ...
                //     color = [0, 1, 1];
                // }
                this.m_lines.push(new VertexData([vertexData__[i], vertexData__[i + 1], vertexData__[i + 2]], Math.floor(Math.random() * 5), [vertexData__[i + 6], vertexData__[i + 7], vertexData__[i + 8]]));
            }
